...
### F. Exec integration (later phase)
...
These indicate Safeclaw already supports contextual restrictions (cwd/allowlist/binding) for risky operations. They are the right place to integrate zone-aware exec restrictions later.

#### MVP recommendation

Do **not** begin with full shell/path analysis.
Start with:

- require `exec.workdir` to resolve inside an authorized zone
- keep existing exec approval/sandbox controls in place
